...
from shared_utils_proposed import add_noise

from dncnn_proposed import DnCNN
...
parser.add_argument('--weights_path', default='weights/proposed_gauss_20_test', type=str, help='path for saving model weights')
parser.add_argument('--model_path', default='saved_models/proposed_gauss_20_test', type=str, help='path for saving whole model')
parser.add_argument('--exp_name', default='proposed_test', type=str, help='name for experiment logs')
...
def augment(image):
    ...
    std = NOISE_STD  # random.randint(0, 55) for blind denoising
    if BLIND_MODE:
        std = random.randint(min_std, NOISE_STD) #for blind denoising
    
    noisy_image,molt_term,chi = add_noise(image,TRAIN_PATCH_DIM,NOISE_FORM,std)


    return noisy_image, image, molt_term,chi
...
def augment_test(image):
    ...
    if BLIND_MODE:
        std = random.randint(min_std, NOISE_STD) #for blind denoising
    noisy_image,molt_term,chi = add_noise(image,TEST_DIM,NOISE_FORM,std)
...
@tf.function
def train_step(x2, y,ye,chi):
    with tf.GradientTape() as tape:
        # training=True is only needed if there are layers with different
        # behavior during training versus inference (e.g. Dropout).
        x2,ye,chi = model(images, training=True)
        loss = loss_object(tf.square(targets)+molt_term, predictions)
        predictions = tf.clip_by_value(predictions, 0, 1)
        metric_psnr = tf.image.psnr(tf.sqrt(predictions), targets, max_val=1.0)
        metric = patch_mean_train(predictions, tf.square(targets))
...
@tf.function
def test_step(images, targets,molt_term):
    # training=False is only needed if there are layers with different
    # behavior during training versus inference (e.g. Dropout).
    predictions = model(images, training=False)
    t_loss = loss_object(tf.square(targets)+molt_term, predictions)
    predictions = tf.clip_by_value(predictions, 0, 1)
    t_metric_psnr = tf.image.psnr(tf.sqrt(predictions), targets, max_val=1.0)
    t_metric = patch_mean_train(predictions, tf.square(targets))
...
# ...
   ...
   # optimizer = tfa.optimizers.AdamW(learning_rate=lr, weight_decay=wd)
    ...
    # training loop
    for images, targets,molt_term in train_ds:
        train_step(images, targets,molt_term)

    # test loop
    for test_images, test_targets, molt_term in test_ds:
        test_step(test_images, test_targets,molt_term)
