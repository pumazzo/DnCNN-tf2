...
from dncnn import DnCNN, DnCNN_RC
from dncnnrn import DnCNNRN
print("loaded modules")
# %%
parser = argparse.ArgumentParser(description='DnCNN tf2')
parser.add_argument('--model', default='test', choices=['DnCNN', 'DnCNNRN','test'], type=str, help='choose a type of model')
...
parser.add_argument('--sigma', default=50, type=int, help='noise level (max level for blind mode)')
...
def train_step(images, targets):
    with tf.GradientTape() as tape:
        ...
        loss = loss_object(tf.math.square(targets), predictions)
        predictions = tf.clip_by_value(predictions, 0, 1)
        metric = tf.image.psnr(predictions, tf.math.square(targets), max_val=1.0)
...
def test_step(images, targets):
    ...
    t_loss = loss_object(tf.math.square(targets), predictions)
    predictions = tf.clip_by_value(predictions, 0, 1)
    t_metric = tf.image.psnr(predictions, tf.math.square(targets), max_val=1.0)
