...
from shared_utils_proposed import gaussian_noise_layer,add_noise
...
parser.add_argument('--weights_path', default='weights/proposed_gauss_20_test', type=str, help='path for loading model weights')
parser.add_argument('--save_plots', action='store_true', help='save plots in plots_dir')
parser.add_argument('--plots_dir', default='plots/proposed', type=str, help='path for saving plots')
...
def augment(image,std):
    ...
    noisy_image,molt_term = add_noise(image,TEST_DIM,NOISE_FORM,std)
    image=tf.square(image)
    return noisy_image, image, molt_term
...
if SAVE_PLOTS:
    ...
    image_batch, target_batch,molt_term = next(iter(test_ds))
